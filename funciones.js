...
function juntar(){
	if(selMul && seleccionMultiple.length>1 && seleccionMultiple.length<3){
		...
		if(n1 != n2){
			//var td = document.getElementById(seleccionMultiple[0]).rowSpan;
			//var td2 = document.getElementById(seleccionMultiple[1]).rowSpan;
			var menor=20;
			var pos=0;
			for(var i = 0;i < seleccionMultiple.length;i++){
...
function guardar(){
	...
	for(var i = 0;i < todos.length; i++){
		gfondo=todos[i].style.background;
		...
		// console.log(gfondo+" "+gsubrayado+" "+gnegrita+" "+gcursiva+" "+galineacion+" "+gcolor+" "+gcol+" "+grow);
		archivo[c]={f:gfondo,s:gsubrayado,n:gnegrita,c:gcursiva,a:galineacion,co:gcolor,col:gcol,row:grow};
		c++;
	}
